...
export default function EditClientModal({ clientId, onClose, onSave }: EditClientModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err.message || (language === 'en' ? 'Failed to update client' : 'Error al actualizar cliente'));
      setSaving(false);
    }
  };
  ...
  if (!canManage) {
    return (
      ...
      >
        ...
        >
          <p>{language === 'en' ? 'You do not have permission to edit clients' : 'No tienes permiso para editar clientes'}</p>
          <button
            onClick={onClose}
            style={{
              marginTop: '1rem',
